...
namespace tira {
    ...
    public:
        // ctor/dtor
        Icosahedron(float radius = 1.0f) : interleavedStride(32) {
            setRadius(radius);
        }
        ~Icosahedron() {}
    ...
    static tmesh icosahedron(float radius = 1.0f) {
        ...
        S.Vertices(&s.vertices[0], s.vertices.size() / 3);
        S.Normals(&s.normals[0], s.normals.size() / 3);
        S.TexCoords(&s.texCoords[0], s.texCoords.size() / 2, 2);
        S.Indices(s.indices);
        return S;
    }
