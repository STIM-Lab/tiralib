...
namespace tira {
    ...
    public:
       ...
       * @param min The minimum allowed length for an edge.
       * @param max The maximum allowed length for an edge.
       ...
       */
        std::vector<size_t> SelectEdgeLength(
            float min, float max,
            const std::vector<size_t>& current = {},
            bool op = false
        ) const {
            std::vector<size_t> result;
            std::vector<bool> selected(m_edges.size(), false);
            for (size_t i : current) if (i < m_edges.size()) selected[i] = true;

            for (size_t ei = 0; ei < m_edges.size(); ++ei) {
                float len = m_edges[ei].Length(
                    m_nodes[m_edges[ei].NodeIndex0()],
                    m_nodes[m_edges[ei].NodeIndex1()]);
                bool in_range = (len >= min && len <= max);

                if (current.empty()) {
                    // no previous query, just do plain selection
                    if (in_range) result.push_back(ei);
                }
                else {
                    if (op) { // AND: keep only if in 'current' and in range
                        if (selected[ei] && in_range) result.push_back(ei);
                    }
                    else {  // OR: keep if in 'current' or in range
                        if (selected[ei] || in_range) result.push_back(ei);
                    }
                }
            }
        ...
        fibernet DeleteEdges(const std::vector<size_t>& edge_indices) const {
            ...
            for (auto& node : new_net.m_nodes) node.ClearEdgeIndices();
        ...
        /**
        * @brief selects all edges whose mean radius falls within the given range [min, max].
        * can be chained with previous results using AND (intersection) or OR (union).
        * @param min     The minimum allowed mean radius for an edge.
        * @param max     The maximum allowed mean radius for an edge.
        * @param current Optional vector of previously selected edge indices.
        * @param op      If true: AND (restrict to edges in 'current' AND in range);
        *                If false: OR (include any edge in 'current' OR in range).
        * @return        A vector of edge indices that satisfy the radius criteria.
        */
        std::vector<size_t> SelectEdgeRadius(
            float min, float max,
            const std::vector<size_t>& current = {},
            bool op = false
        ) const {
